...
def detect(save_img=False):
    ...
    save_dir = Path('./autocropped', exist_ok=opt.exist_ok)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            # save_path = str(save_dir / source.split('/')[-3] / p.name)  # img.jpg
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if opt.prompt in label:
                            x0, y0, x1, y1 = int(xyxy[0]), int(xyxy[1]), int(xyxy[2]), int(xyxy[3])
                            # print(im0.shape)
                            # cv2.imwrite('yolo_bounding_box.jpg', im0[y0:y1, x0:x1])
...
# python3 crop.py --weights yolov7.pt --conf 0.25 --source "./test.jpg" --prompt dog --no-trace --device cpu
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prompt', help='class to detect and crop')
